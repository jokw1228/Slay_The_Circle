...
static func create(radius_to_set: float, radian_to_set: float, to_rotate: float) -> BackGroundArcDrawer:
	var inst: BackGroundArcDrawer = load(BackGroundArcDrawer_scene).instantiate() as BackGroundArcDrawer
...
func _draw():
	...
	draw_arc(Vector2.ZERO, radius, 0, radian, 24, arc_color, arc_width, true)
